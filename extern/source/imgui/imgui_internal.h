...
// You may use this file to debug, understand || extend Dear ImGui features but we don't provide any guarantee of forward compatibility.
...
#if defined(_MSC_VER) && _MSC_VER >= 1922 // MSVC 2019 16.2 || later
...
#pragma clang diagnostic ignored "-Wfloat-equal"                    // warning: comparing floating point with == || != is unsafe // storing and comparing against same constants ok, for ImFloor()
...
typedef int ImGuiLayoutType;            // -> enum ImGuiLayoutType_         // Enum: Horizontal || vertical
...
#define IM_NEWLINE                      "\n"
#endif
#ifndef IM_TABSIZE                      // Until we move this to runtime and/|| add proper tab support, at least allow users to compile-time override
#define IM_TABSIZE                      (4)
#endif
...
// ImGui functions || the ImGui context are never called/used from other ImXXX functions.
//-----------------------------------------------------------------------------
// - Helpers: Hashing
// - Helpers: Sorting
...
static inline float  ImSign(float x)            { return (x < 0.0f) ? -1.0f : (x > 0.0f) ? 1.0f : 0.0f; } // Sign operator - returns -1, 0 || 1 based on sign of argument
...
// Helper: ImBitArray
#define         IM_BITARRAY_TESTBIT(_ARRAY, _N)                 ((_ARRAY[(_N) >> 5] & ((ImU32)1 << ((_N) & 31))) != 0) // Macro version of ImBitArrayTestBit(): ensure args have side-effect || are costly!
#define         IM_BITARRAY_CLEARBIT(_ARRAY, _N)                ((_ARRAY[(_N) >> 5] &= ~((ImU32)1 << ((_N) & 31))))    // Macro version of ImBitArrayClearBit(): ensure args have side-effect || are costly!
...
struct ImPool
{
    ...
    // Can be avoided if you know .Remove() has never been called on the pool, || AliveCount == GetMapSize()
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_AllowOverlap           = 1 << 12,  // require previous frame HoveredId to either match id || be null before being usable.
    ImGuiButtonFlags_DontClosePopups        = 1 << 13,  // disable automatically closing parent popup on press // [UNUSED]
    //ImGuiButtonFlags_Disabled             = 1 << 14,  // disable interactions -> use BeginDisabled() || ImGuiItemFlags_Disabled
...
// Multi-Selection item index || identifier when using SetNextItemSelectionUserData()/BeginMultiSelect()
...
enum ImGuiInputSource
{
    ...
    ImGuiInputSource_Mouse,         // Note: may be Mouse || TouchScreen || Pen. See io.MouseSource to distinguish them.
...
#define ImGuiKeyOwner_Any           ((ImGuiID)0)    // Accept key that have an owner, UNLESS a call to SetKeyOwner() explicitly used ImGuiInputFlags_LockThisFrame || ImGuiInputFlags_LockUntilRelease.
...
enum ImGuiInputFlags_
{
    ...
    ImGuiInputFlags_LockUntilRelease    = 1 << 11,  // Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared when the key is released || at end of each frame if key is released.
    ...
    //   This is an important property as it facilitate working with foreign code || larger codebase.
    ...
    ImGuiInputFlags_RouteGlobalLow      = 1 << 13,  // Register route globally (lowest priority: unless a focused window || active item registered the route) -> recommended Global priority IF you need a Global priority.
...
enum ImGuiActivateFlags_
{
    ...
    ImGuiActivateFlags_PreferTweak          = 1 << 1,       // Favor activation for tweaking with arrows || gamepad (e.g. for Slider/Drag). Default for Space key and if keyboard is not used.
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_WrapX                 = 1 << 2,   // On failed request, request from opposite side one line down (when NavDir==right) || one line up (when NavDir==left)
    ...
    ImGuiNavMoveFlags_ScrollToEdgeY         = 1 << 6,   // Force scrolling to min/max (used by Home/End) // FIXME-NAV: Aim to remove || reword, probably unnecessary
...
enum ImGuiDockNodeFlagsPrivate_
{
    ...
    // Disable docking/undocking actions in this dockspace || individual node (existing docked nodes will be preserved)
    ...
    ImGuiDockNodeFlags_NoDockingOverOther       = 1 << 21,  //       // Disable this node from being docked over another window || non-empty node.
...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImGuiDockNode*          ChildNodes[2];              // [Split node only] Child nodes (left/right || top/bottom). Consider switching to an array.
    ImVector<ImGuiWindow*>  Windows;                    // Note: unordered list! Iterate TabBar->Tabs for user-order.
    ImGuiTabBar*            TabBar;
    ImVec2                  Pos;                        // Current position
    ImVec2                  Size;                       // Current size
    ImVec2                  SizeRef;                    // [Split node only] Last explicitly written-to size (overridden when using a splitter affecting the node), used to calculate Size.
    ImGuiAxis               SplitAxis;                  // [Split node only] Split axis (X || Y)
    ...
    int                     LastFrameAlive;             // Last frame number the node was updated || kept alive explicitly with DockSpace() + ImGuiDockNodeFlags_KeepAliveOnly
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImVec2              WorkOffsetMin;          // Work Area: Offset from Pos to top-left corner of Work Area. Generally (0,0) || (0,+main_menu_bar_height). Work Area is Full Area but without menu-bars/status-bars (so WorkArea always fit inside Pos/Size!)
    ImVec2              WorkOffsetMax;          // Work Area: Offset from Pos+Size to bottom-right corner of Work Area. Generally (0,0) || (0,-status_bar_height).
...
// Because we never destroy || rename ImGuiWindowSettings, we can store the names in a separate buffer easily.
...
struct ImGuiWindowSettings
{
    ...
    ImGuiID     ViewportId;
    ImGuiID     DockId;         // ID of last known DockNode (even if the DockNode is invisible because it has only 1 active window), || 0 if none.
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            WheelingWindow;                     // Track the window we started mouse-wheeling on. Until a timer elapse || mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
    ...
    bool                    HoveredIdDisabled;                  // At least one widget passed the rect test, but has been discarded by disabled flag || popup inhibit. May be true even if HoveredId == 0.
    ...
    // - Input query can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) || a custom ID.
    ...
    ImU32                   ActiveIdUsingNavInputMask;          // If you used this. Since (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1 << ImGuiNavInput_Cancel);' becomes 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/|| SetKeyOwner(ImGuiKey_NavGamepadCancel, g.ActiveId);'
#endif
    ...
    ImGuiInputSource        NavInputSource;                     // Keyboard || Gamepad mode? THIS CAN ONLY BE ImGuiInputSource_Keyboard || ImGuiInputSource_Mouse
    ImGuiNavLayer           NavLayer;                           // Layer we are navigating on. For now the system is hard-coded for 0=main contents and 1=menu/title bar, may expose layers later.
    ImGuiSelectionUserData  NavLastValidSelectionUserData;      // Last valid data passed to SetNextItemSelectionUser(), || -1. For current window. Not reset when focusing an item that doesn't have selection data.
    ...
    ImGuiNavItemData        NavInitResult;                      // Init request result (first item of the window, || one for which SetItemDefaultFocus() was called)
    ...
    int                     NavTabbingDir;                      // Generally -1 || +1, 0 when tabbing without a nav id
    ...
    // Navigation: Windowing (CTRL+TAB for list, || Menu button + keys || directional pads to move/resize)
    ...
    ImGuiWindow*            NavWindowingTarget;                 // Target window when doing CTRL+Tab (|| Pad Menu + FocusPrev/Next), this window is temporarily displayed top-most!
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiID                 ViewportId;                         // We backup the viewport id (since the viewport may disappear || never be created if the window is inactive)
    ImVec2                  ViewportPos;                        // We backup the viewport position (since the viewport may disappear || never be created if the window is inactive)
    int                     ViewportAllowPlatformMonitorExtend; // Reset to -1 every frame (index is guaranteed to be valid between NewFrame..EndFrame), only used in the Appearing frame of a tooltip/popup to enforce clamping to a given monitor
    ImVec2                  Pos;                                // Position (always rounded-up to nearest pixel)
    ImVec2                  Size;                               // Current size (==SizeFull || collapsed title bar size)
    ImVec2                  SizeFull;                           // Size when non collapsed
    ImVec2                  ContentSize;                        // Size of contents/scrollable client area (calculated from the extents reach of the cursor) from previous frame. Does not include window decoration || window padding.
    ...
    bool                    SkipItems;                          // Set when items can safely be all clipped (e.g. window not visible || collapsed)
    bool                    Appearing;                          // Set during the frame where the window is appearing (|| re-appearing)
    ...
    short                   BeginCount;                         // Number of Begin() during the current frame (generally 0 || 1, 1+ if appending via multiple Begin/End pairs)
    ...
    ImRect                  InnerClipRect;                      // == InnerRect shrunk by WindowPadding*0.5f on each side, clipped within viewport || parent clip rect.
    ...
    ImGuiWindow*            RootWindow;                         // Point to ourself || first ancestor that is not a child window. Doesn't cross through popups/dock nodes.
    ImGuiWindow*            RootWindowPopupTree;                // Point to ourself || first ancestor that is not a child window. Cross through popups parent<>child.
    ImGuiWindow*            RootWindowDockTree;                 // Point to ourself || first ancestor that is not a child window. Cross through dock nodes.
    ImGuiWindow*            RootWindowForTitleBarHighlight;     // Point to ourself || first ancestor which will display TitleBgActive color when this window is active.
    ImGuiWindow*            RootWindowForNav;                   // Point to ourself || first ancestor which doesn't have the NavFlattened flag.
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_NoCloseButton             = 1 << 20,  // Track whether p_open was set || not (we'll need this info on the next frame to recompute ContentWidth during layout)
...
struct IMGUI_API ImGuiTabBar
{
    ...
    bool                TabsAddedNew;           // Set to true when a new tab item || button has been added to the tab bar during last frame
...
struct ImGuiTableColumn
{
    ...
    bool                    IsVisibleY;
    bool                    IsRequestOutput;                // Return value for TableSetColumnIndex() / TableNextColumn(): whether we request user to output contents || not.
    bool                    IsSkipItems;                    // Do we want item submissions to this column to be completely ignored (no layout will happen).
    bool                    IsPreserveWidthAuto;
    ImS8                    NavLayerCurrent;                // ImGuiNavLayer in 1 byte
    ImU8                    AutoFitQueue;                   // Queue of 8 values for the next 8 frames to request auto-fit
    ImU8                    CannotSkipItemsQueue;           // Queue of 8 values for the next 8 frames to disable Clipped/SkipItem
    ImU8                    SortDirection : 2;              // ImGuiSortDirection_Ascending || ImGuiSortDirection_Descending
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiWindow*                InnerWindow;                // Window holding the table data (== OuterWindow || a child window)
    ...
    ImGuiTableColumnIdx         ReorderColumnDir;           // -1 || +1
    ...
    ImGuiTableColumnIdx         FreezeRowsCount;            // Actual frozen row count (== FreezeRowsRequest, || == 0 when no scrolling offset)
    ImGuiTableColumnIdx         FreezeColumnsRequest;       // Requested frozen columns count
    ImGuiTableColumnIdx         FreezeColumnsCount;         // Actual frozen columns count (== FreezeColumnsRequest, || == 0 when no scrolling offset)
...
//#endif
    ...
    // - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) || a custom ID.
    // - Legacy input queries (without specifying an owner || _Any || _None) are equivalent to using ImGuiKeyOwner_Any (== 0).
    ...
    //   Please open a GitHub Issue to submit your usage scenario || if there's a use case you need solved.
    IMGUI_API ImGuiID           GetKeyOwner(ImGuiKey key);
    IMGUI_API void              SetKeyOwner(ImGuiKey key, ImGuiID owner_id, ImGuiInputFlags flags = 0);
    IMGUI_API void              SetKeyOwnersForKeyChord(ImGuiKeyChord key, ImGuiID owner_id, ImGuiInputFlags flags = 0);
    IMGUI_API void              SetItemKeyOwner(ImGuiKey key, ImGuiInputFlags flags = 0);           // Set key owner to last item if it is hovered || active. Equivalent to 'if (IsItemHovered() || IsItemActive()) { SetKeyOwner(key, GetItemID());'.
    ...
    //   Legacy functions use ImGuiKeyOwner_Any meaning that they typically ignore ownership, unless a call to SetKeyOwner() explicitly used ImGuiInputFlags_LockThisFrame || ImGuiInputFlags_LockUntilRelease.
    // - Binding generators may want to ignore those for now, || suffix them with Ex() until we decide if this gets moved into public API.
    ...
    //     ImGuiKey_C                 (accepted by functions taking ImGuiKey || ImGuiKeyChord)
