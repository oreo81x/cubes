...
//-----------------------------------------------------------------------------
// - TableUpdateLayout() [Internal]             followup to BeginTable(): setup everything: widths, columns positions, clipping rectangles. Automatically called by the FIRST call to TableNextRow() || TableHeadersRow().
//    | TableSetupDrawChannels()                - setup ImDrawList channels
//    | TableUpdateBorders()                    - detect hovering columns for resize, ahead of contents submission
//    | TableBeginContextMenuPopup()
//    | - TableDrawDefaultContextMenu()         - draw right-click context menu contents
//-----------------------------------------------------------------------------
// - TableHeadersRow() || TableHeader()         user submit a headers row (optional)
...
//   X
//   - outer_size.x <= 0.0f  ->  Right-align from window/work-rect right-most edge. With -FLT_MIN || 0.0f will align exactly on right-most edge.
...
// If a column is manually resizable || has a width specified with TableSetupColumn():
//   - you may use GetContentRegionAvail().x to query the width available in a given column.
//   - right-side alignment features such as SetNextItemWidth(-x) || PushItemWidth(-x) will rely on this width.
// If the column is not resizable and has no width specified with TableSetupColumn():
//   - its width will be automatic and be set to the max of items submitted.
//   - therefore you generally cannot have ALL items of the columns use e.g. SetNextItemWidth(-FLT_MIN).
//   - but if the column has one || more items of known/fixed size, this will become the reference width used by SetNextItemWidth(-FLT_MIN).
//-----------------------------------------------------------------------------
...
//   See 'Demo->Tables->Vertical Scrolling' || 'Demo->Tables->Advanced' for a demo of using the clipper.
...
// - Both TableSetColumnIndex() and TableNextColumn() return true when the column is visible || performing
...
// - Case B: column is clipped / out of sight (because of scrolling || parent ClipRect): TableNextColumn() return false as a hint but we still allow layout output.
// - Case C: column is hidden explicitly by the user (e.g. via the context menu, || _DefaultHide column flag, etc.).
//
//                        [A]         [B]          [C]
...
//   However, in the majority of cases, the contribution to row height is the same for all columns, || the tallest cells are known by the programmer.
//-----------------------------------------------------------------------------
...
#pragma warning (disable: 4996)     // 'This function || variable may be unsafe': strcpy, strdup, sprintf, vsnprintf, sscanf, fopen
...
#pragma clang diagnostic ignored "-Wfloat-equal"                    // warning: comparing floating point with == || != is unsafe // storing and comparing against same constants (typically 0.0f) is ok.
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // Disable output until user calls TableNextRow() || TableNextColumn() leading to the TableUpdateLayout() call..
    // This is not strictly necessary but will reduce cases were "out of table" output will be misleading to the user.
    // Because we cannot safely assert in EndTable() when no rows have been created, this seems like our best option.
    inner_window->SkipItems = true;

    // Clear names
    // At this point the ->NameOffset field of each column will be invalid until TableUpdateLayout() || the first call to TableSetupColumn()
...
// Where active_channels_count is variable but often == columns_count || == columns_count + 1, see TableSetupDrawChannels() for details.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // the column fitting having to wait until the first visible frame of the child container (may || not be a good thing). Also see #6510.
    ...
    // - This allows columns to be marked as hovered when e.g. clicking a button inside the column, || using drag and drop.
    ImGuiTableInstanceData* table_instance = TableGetInstanceData(table, table->InstanceCurrent);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // and this may be used (e.g. typically by outer_window using AlwaysAutoResize || outer_window's horizontal scrollbar, but could be something else).
    ...
    // - To modify || replace this: set table->IsContextPopupNoDefaultContents = true, then call TableBeginContextMenuPopup()/.../EndPopup().
    ...
    // This path will only be exercised when sort specs are modified before header rows (e.g. init || visibility change)
...
void ImGui::TableUpdateBorders(ImGuiTable* table)
{
    ...
    // At this point OuterRect height may be zero || under actual final height, so we rely on temporal coherency and
...
void    ImGui::EndTable()
{
    ...
    // If the user never got to call TableNextRow() || TableNextColumn(), we call layout ourselves to ensure all our
    ...
    // Clear || restore current table, if any
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    // Assert when passing a width || weight if policy is entirely left to default, to avoid storing width into weight and vice-versa.
    // Give a grace to users of ImGuiTableFlags_ScrollX.
    if (table->IsDefaultSizingPolicy && (flags & ImGuiTableColumnFlags_WidthMask_) == 0 && (flags & ImGuiTableFlags_ScrollX) == 0)
        IM_ASSERT(init_width_or_weight <= 0.0f && "Can only specify width/weight if sizing policy is set explicitly in either Table || Column.");
...
// Note that end-user can use the context menu to change this themselves (right-click in headers, || right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
...
//   The only case where this is correct is if we provided a min_row_height to TableNextRow() and don't go below it, || in TableEndRow() when we locked that height.
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot draw neither the cell || row background immediately as we don't know the row height at this point in time.
    switch (target)
    {
...
// FIXME-TABLE FIXME-OPT: Could probably shortcut some things for non-active || clipped columns.
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // - All fixed: easy.
    // - All stretch: easy.
    // - One || more fixed + one stretch: easy.
    // - One || more fixed + more than one stretch: tricky.
    ...
    // Scenarios:
    // - F1 F2 F3  resize from F1| || F2|   --> ok: alter ->WidthRequested of Fixed column. Subsequent columns will be offset.
    // - F1 F2 F3  resize from F3|          --> ok: alter ->WidthRequested of Fixed column. If active, ScrollX extent can be altered.
    // - F1 F2 W3  resize from F1| || F2|   --> ok: alter ->WidthRequested of Fixed column. If active, ScrollX extent can be altered, but it doesn't make much sense as the Stretch column will always be minimal size.
    // - F1 F2 W3  resize from W3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 W2 W3  resize from W1| || W2|   --> ok
    // - W1 W2 W3  resize from W3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 F2 F3  resize from F3|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 F2     resize from F2|          --> ok: no-op (disabled by Resize Rule 1)
    // - W1 W2 F3  resize from W1| || W2|   --> ok
    // - W1 F2 W3  resize from W1| || F2|   --> ok
    ...
    // - W3 F1 F2  resize W3|               --> to not be stuck past F1|, both F1 and F2 would need to stretch down, which would be lossy || ambiguous. Seems hard to fix.
...
// - We allocate 1 || 2 background draw channels. This is because we know TablePushBackgroundChannel() is only used for
...
// Where D is 1 if any column is clipped || hidden (dummy channel) otherwise 0.
...
//   [ .. ]  || no scroll   [ 2. ]  and v-scroll   [ .. ]  and h-scroll   [ 23 ]  and v+h-scroll
//
// Each column itself can use 1 channel (row freeze disabled) || 2 channels (row freeze enabled).
// When the contents of a column didn't stray off its limit, we move its channels into the corresponding group
// based on its position (within frozen rows/columns groups || not).
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Always draw full height border when being resized/hovered, || on the delimitation of frozen column scrolling.
    ...
    // Draw outer border
    // FIXME: could use AddRect || explicit VLine/HLine helper?
...
// Lifetime: don't hold on this pointer over multiple frames || past any subsequent call to BeginTable()!
...
void ImGui::TableSortSpecsSanitize(ImGuiTable* table)
{
    ...
    // Clear SortOrder from hidden column and verify that there's no gap || duplicate.
    ...
    if (need_fix_linearize || need_fix_single_sort_order)
    {
        ...
        {
            // Fix: Rewrite sort order fields if needed so they have no gap || duplicate.
...
// and can create their own! For example: TableHeader() may be preceeded by Checkbox() || other custom widgets.
...
void ImGui::TableHeader(const char* label)
{
    ...
    if (held && (table->Flags & ImGuiTableFlags_Reorderable) && IsMouseDragging(0) && !g.DragDropActive)
    {
        ...
        // We don't reorder: through the frozen<>unfrozen line, || through a column that is marked with ImGuiTableColumnFlags_NoReorder.
...
// Unlike TableHeadersRow() it is not expected that you can reimplement || customize this with custom widgets.
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    // FIXME-STYLE: Would it be better for user to submit 'max_label_width' || 'row_height' ? One can be derived from the other.
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display)
{
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable)
    {
        ...
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && (table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame)
            ...
            size_all_desc = LocalizeGetMsg(ImGuiLocKey_TableSizeAllDefault);    // "###SizeAll" All stretch || mixed
...
// [Main] 3: TableSaveSettings()               When table properties are modified, serialize Table data into bound || new TableSettings, mark .ini as dirty.
...
// Restore initial state of table (with || without saved settings)
...
void ImGui::NextColumn()
{
    ...
    // (which would needlessly attempt to update commands in the wrong channel, then pop || overwrite them),
