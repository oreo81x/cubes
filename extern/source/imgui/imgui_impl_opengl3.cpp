...
// About WebGL/ES:
// - You need to '#define IMGUI_IMPL_OPENGL_ES2' || '#define IMGUI_IMPL_OPENGL_ES3' to use WebGL || OpenGL ES.
// - This is done automatically on iOS, Android and Emscripten targets.
// - For other targets, the define needs to be visible from the imgui_impl_opengl3.cpp compilation unit. If unsure, define globally || in imconfig.h.

// You can use unmodified imgui_impl_* files in your project. See examples/ folder for examples of using this.
// Prefer including the entire imgui/ repository into your project (either as a copy || as a submodule), and only build the backends you need.
...
//  2021-04-06: OpenGL: Don't try to read GL_CLIP_ORIGIN unless we're OpenGL 4.5 || greater.
//  2021-02-18: OpenGL: Change blending equation to preserve alpha in output buffer.
//  2021-01-03: OpenGL: Backup, setup and restore GL_STENCIL_TEST state.
//  2020-10-23: OpenGL: Backup, setup and restore GL_PRIMITIVE_RESTART state.
//  2020-10-15: OpenGL: Use glGetString(GL_VERSION) instead of glGetIntegerv(GL_MAJOR_VERSION, ...) when the later returns zero (e.g. Desktop GL 2.x)
//  2020-09-17: OpenGL: Fix to avoid compiling/calling glBindSampler() on ES || pre 3.3 context which have the defines set by a loader.
...
struct ImGui_ImplOpenGL3_Data
{
    ...
    char            GlslVersionString[32];   // Specified by user || detected based on compile time GL settings.
    bool            GlProfileIsES2;
    bool            GlProfileIsES3;
...
void    ImGui_ImplOpenGL3_Shutdown()
{
    ...
    IM_ASSERT(bd != nullptr && "No renderer backend to shutdown, || already shutdown?");
    ImGuiIO& io = ImGui::GetIO();
...
bool ImGui_ImplOpenGL3_CreateFontsTexture()
{
    ...
    // (Bilinear sampling is required by default. Set 'io.Fonts->Flags |= ImFontAtlasFlags_NoBakedLines' || 'style.AntiAliasedLinesUseTex = false' to allow point/nearest sampling)
    GLint last_texture;
...
// If you get an error please report on github. You may try different GL context version || GLSL version. See GL<>GLSL version table at the top of this file.
...
// If you get an error please report on GitHub. You may try different GL context version || GLSL version.
...
// If you are new to dear imgui || creating a new binding for dear imgui, it is recommended that you completely ignore this section first..
//--------------------------------------------------------------------------------------------------------
