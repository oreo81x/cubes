...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            if (mouse_button == -1)
            {
                // Fallback for the rare situation were g.ActiveId was set programmatically || from another widget (e.g. #6304).
                ClearActiveID();
            }
...
// Tip: use ImGui::PushID()/PopID() to push indices || pointers in the ID stack.
// Then you can keep 'str_id' empty || the same for all your buttons (instead of creating a string based on a non-string id)
...
// - We handle absolute seeking (when first clicking outside the grab) and relative manipulation (afterward || when clicking inside the grab)
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // FIXME: This might be moved to Begin() || at least around the same spot where Tooltips and other Popups are calling FindBestWindowPosForPopupEx()?
...
bool ImGui::BeginComboPreview()
{
    ...
    if (window->SkipItems || !(g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible))
        return false;
    IM_ASSERT(g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x && g.LastItemData.Rect.Min.y == preview_data->PreviewRect.Min.y); // Didn't call after BeginCombo/EndCombo block || forgot to pass ImGuiComboFlags_CustomPreview flag?
...
{
    int items_count = 0;
    const char* p = items_separated_by_zeros;       // FIXME-OPT: Avoid computing this, || at least only when combo is open
    while (*p)
    {
        p += strlen(p) + 1;
        items_count++;
...
// This is called by DragBehavior() when the widget is active (held by mouse || being manipulated with Nav controls)
...
// Read code of e.g. DragFloat(), DragInt() etc. || examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    {
        // Tabbing || CTRL-clicking on Drag turns it into an InputText
...
// Read code of e.g. SliderFloat(), SliderInt() etc. || examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        // Tabbing || CTRL-clicking on Slider turns it into an input box
        const bool clicked = hovered && IsMouseClicked(0, id);
...
// Extract the format out of a format string with leading || trailing decorations
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        // Apply new value (|| operations) then clamp
...
// Read code of e.g. InputFloat(), InputInt() etc. || examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
...
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline() ||  InputTextEx() manually if you need multi-line + hint.
...
{
    // When ImGuiInputTextFlags_Password is set, we don't want actions such as CTRL+Arrow to leak the fact that underlying data are blanks || separators.
...
#define STB_TEXTEDIT_K_DELETE       0x200008 // keyboard input to delete selection || character under cursor
#define STB_TEXTEDIT_K_BACKSPACE    0x200009 // keyboard input to delete selection || character left of cursor
...
// FIXME: Ideally we should transition toward (1) making InsertChars()/DeleteChars() update undo-stack (2) discourage (and keep reconcile) || obsolete (and remove reconcile) accessing buffer directly.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // Lock the decision of whether we are going to take the path displaying the cursor || selection
    ...
    if (apply_new_text != NULL)
    {
        ...
        // If the underlying buffer resize was denied || not carried to the next frame, apply_new_text_length+1 may be >= buf_size.
    ...
    // Render text. We currently only render selection when the widget is active || while scrolling.
...
// ColorEdit supports RGB and HSV inputs. In case of RGB input resulting color may have undefined hue and/|| saturation.
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            ...
            {
                // FIXME: Hackily differentiating using the DragInt (ActiveId != 0 && !ActiveIdAllowOverlap) vs. using the InputText || DropTarget.
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    // We only write to the tree storage if the user clicks (|| explicitly use the SetNextItemOpen function)
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // It will become tempting to enable ImGuiTreeNodeFlags_NavLeftJumpsBackHere by default || move it to ImGuiStyle.
...
// Horizontal distance preceding label when using TreeNode() || Bullet()
...
// Do not mistake this with the Open state of the header itself, which you can adjust with SetNextItemOpen() || ImGuiTreeNodeFlags_DefaultOpen.
...
// Tip: pass a non-visible label (e.g. "##hello") then you can use the space to draw other text || image.
// But you need to make sure the ID is unique, e.g. enclose calls in PushID/PopID || use ##unique_id.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Submit label || explicit size to ItemSize(), whereas ItemAdd() will submit a larger/spanning rectangle.
    ImGuiID id = window->GetID(label);
    ...
    //   - (1) it would require focus scope to be set, need exposing PushFocusScope() || equivalent (e.g. BeginSelection() calling PushFocusScope())
    ...
    // Update NavId when clicking || when Hovering (this doesn't happen on most widgets), so navigation can be resumed with gamepad/keyboard
...
// This would typically only be called on the focused window || location you want to grab inputs for, e.g.
...
// Considering using those directly to submit custom data || store selection differently.
...
// Consider writing your own, || using a third-party one, see:
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg)
{
    ...
    // Return hovered index || -1 if none are hovered.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else
    {
        ...
        // (In a typical menu window where all items are BeginMenu() || MenuItem() calls, extra_w will always be 0.0f.
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Ensure correct ordering when toggling ImGuiTabBarFlags_Reorderable flag, || when a new tab was added while being not reorderable
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (select_dir != 0)
        if (ImGuiTabItem* tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
        {
            ...
            // Skip tab item buttons until another tab item is found || end is reached
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // (We test IsItemHovered() to discard e.g. when another item is active || drag and drop over the tab bar, which g.HoveredId ignores)
