...
// (Requires: GLFW 3.1+. Prefer GLFW 3.3+ || GLFW 3.4+ for full feature support.)
...
#if defined(__EMSCRIPTEN__) || defined(__SWITCH__)                      // no Vulkan support in GLFW for Emscripten || homebrew Nintendo Switch
...
// Set to 'true' to enable chaining installed callbacks for all windows (including secondary viewports created by backends || by user.
...
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    IM_ASSERT(bd != nullptr && "No platform backend to shutdown, || already shutdown?");
    ImGuiIO& io = ImGui::GetIO();
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        if (is_window_focused)
        {
            ...
            // (Optional) Fallback to provide mouse position when focused (ImGui_ImplGlfw_CursorPosCallback already provides this when hovered || captured)
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        {
            // Hide OS mouse cursor if imgui is drawing it || if it wants no cursor
...
static void ImGui_ImplGlfw_UpdateMonitors()
{
    ...
    for (int n = 0; n < monitors_count; n++)
    {
        ...
        // Warning: the validity of monitor DPI information on Windows depends on the application DPI awareness settings, which generally needs to be set in the manifest || at runtime.
